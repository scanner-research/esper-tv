...
def export_table(t):
    ...
    p = pexpect.spawn("""bash -c "echo 'select * from query_tvnews_{}' | psql -h db esper will -t -F ',' -A > {}""".format(t, csv_path(t)), timeout=None)
...
class TableLoader:
    def video(self, rows):
        ...
        for row in rows:
            ...
            models.append(Video(
                id=int(id), path=path, num_frames=int(num_frames), fps=float(fps), width=int(width), 
                height=int(height), channel=channel, show=show, time=datetime.strptime(dt, '%Y-%m-%d %H:%M:%S+00')))
    ...
    def pose(self, rows):
        ...
        for i, row in enumerate(rows):
            [id, bbox_x1, bbox_x2, bbox_y1, bbox_y2, bbox_score, frame_id, labeler_id, track_id, keypoints] = row
            poses.append(Pose(
                id=int(id), keypoints=parse_hex(keypoints), labeler_id=int(labeler_id), 
                person=people[i]))
    ...
    def facetrack(self, rows):
        ...
        for row in rows:
            ...
            models.append(
                PersonTrack(id=int(id), video_id=int(video_id), min_frame=int(min_frame), 
                            labeler=labeler,
    ...
    def face(self, rows):
        ...
        genders = []
        for i, row in enumerate(rows):
            [id, track_id, frame_id, labeler_id, bbox_score, bbox_x1, bbox_x2, bbox_y1, bbox_y2, gender_id, identity_id] = row
            genders.append(int(gender_id))
            people[i].tracks.add(track_id)
            faces.append(Face(
                id=int(id), bbox_x1=float(bbox_x1), bbox_x2=float(bbox_x2), bbox_y1=float(bbox_y1), 
                bbox_y2=float(bbox_y2), bbox_score=float(bbox_score), labeler_id=int(labeler_id), 
                person=people[i]))
        ...
        batch_create(
            FaceGender,
            [FaceGender(face=face, gender_id=gender_id, labeler=gender_labeler) for face, gender_id in zip(faces, genders)])

    def facefeatures(self, rows):
        batch_create(
            FaceFeatures, 
            [FaceFeatures(
                id=int(id), features=parse_hex(features), face_id=int(face_id), 
                labeler_id=int(labeler_id)) 
             for [id, features, face_id, labeler_id, _] in rows])
