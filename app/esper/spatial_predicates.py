...
def and_pred_spatial(pred1, pred2):
    return lambda tr: pred1(tr) and pred2(tr)
def or_pred_spatial(pred1, pred2):
    return lambda tr: pred1(tr) or pred2(tr)

'''Cardinal predicates'''
def exactly(n):
    return lambda tr: len(tr.payload['objects']) == n
def at_least(n):
    return lambda tr: len(tr.payload['objects']) >= n
def at_most(n):
    return lambda tr: len(tr.payload['objects']) <= n
def between(n1, n2):
    return lambda tr: (len(tr.payload['objects']) >= n1 and
            len(tr.payload['objects']) <= n2)
